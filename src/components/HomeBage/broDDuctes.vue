<template>
  <v-container fluid>
    <v-row class="py-10" style="background-color: #e8e8e8ff">
      <v-col
        class="px-0 mx-2 d-flex justify-center align-center"
        style="gap: 10px"
        max-
        v-for="(data, i) in svgarr"
        :key="i"
      >
        <v-card
          elevation="0"
          class="py-10"
          style="height: 300px; max-width: 350px; min-width: 200px; !important"
        >
          <v-card-text>
            <div class="img-svg"><span v-html="data.svg"></span></div>
          </v-card-text>
          <v-card-title style="font-size: 16px; font-weight: bold">{{
            data.title
          }}</v-card-title>
          <v-card-text style="font-size: 12px">{{ data.text }}</v-card-text>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
      ...
      svgarr: [
        {
          ...
          title: "Qualiity and saving",
          text: "doloremque quibusdam ut tempora unde consequuntur quod voluptatem placeat nemo soluta? Dolores, quidem",
        },
        {
